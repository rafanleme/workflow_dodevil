...
Hab="HM_Menu";Hac="HM_Item";Had="HM_Array";Hl3="HM_Img";Hl2=new RegExp("^("+Hl3+"\\d+).*$");Function.prototype.gk=true;Function.prototype.gm=false;String.prototype.gk=false;String.prototype.gm=true;String.prototype.gn=false;String.prototype.gl=false;
Number.prototype.gm=false;Number.prototype.gk=false;Number.prototype.gn=false;Number.prototype.gl=true;Boolean.prototype.gm=false;Boolean.prototype.gk=false;Boolean.prototype.gn=true;Boolean.prototype.gl=false;Array.prototype.gp=false;Array.prototype.k6=true;
...
HWa=HXI;
function HWD(iv,disptext,hasmore){var gs=Hbg.cw?(gv.hi)?(Hbf.k-Hu-Hw):(Hbf.k-Hu):Hbf.cc;var cn="<DIV ID='"+iv+"' STYLE='position:absolute;top:0;left:0;width:"+gs+"px'>";if(hasmore&&Hbg.cx){var jb=Hbf.R?HA:Hy;if(typeof(Hgi)=="number"){var gu="top:"+(Ht+Hgi)+"px;width:"+HC+"px;height:"+Hl5+"px;";}else{var gu="top:"+Ht+"px;width:"+HC+"px;height:"+Hl5+"px;";}var jc="<IMG ID='HM_ImMore' STYLE='position:absolute;"+gu+"' WIDTH='"+HC+"' HEIGHT='"+Hl5+"' SRC='"+jb+"' HSPACE=0 VSPACE=0 BORDER=0>";cn+=jc;
}cn+=disptext+"</DIV>";return cn;}
...
this.HX9=HWZ;this.I=null;this.HXP=HXO;}
...
Hbg.HXq=HWJ;Hbg.onselectstart=HWL;Hbg.moveTo=HWK;Hbg.cz=((Hbg.cv&&Hbf.cl)||(!Hbg.cv&&Hbf.ck));Hbg.gy="<DIV ID='"+Hab+cq+"' STYLE='position:absolute;visibility:hidden;top:-2000;left:-500;width:"+Hbf.k+"px'>";if(HV){Hbg.gy+="<DIV STYLE='position:absolute;visibility:inherit;overflow:hidden;left:0;top:0;width:100%'>";}}while(Hbg.bo<Hbg.cy){Hbg.bo++;Hbh=Hg.document.all(Hac+cq+"_"+Hbg.bo);if(!Hbh){if(HN)window.status="Creating Hierarchical Menus:"+cq+"/"+Hbg.bo;Hbg.gx=HWF(cq);Hbg.gy+=Hbg.gx.gz;
...
function HWJ(iz){if(this.cw){var hk=0;for(i=0;i<this.items.length;i++){var el=this.items[i];if(el.index){var lt=el.scrollHeight;hk=Math.max(hk,lt);}else{hk=el.scrollHeight;}}for(i=0;i<this.items.length;i++){var el=this.items[i];el.style.pixelHeight=hk;if(el.hg){if(this.tree.R){el.hg.style.pixelLeft=(Ht+HD);}else{el.hg.style.pixelLeft=el.style.pixelWidth-((el.hi?0:Hw)+Ht+HD+HC);}if(typeof(Hgi)!="number"){if(Hgi=="bottom"){el.hg.style.pixelTop=Math.max((hk-Hl5-Ht),Ht);
}else{el.hg.style.pixelTop=parseInt(Math.max((hk-Hl5),Ht)/2);}}}}this.style.pixelHeight=this.cs.style.pixelHeight=(hk+(Hu*2));}else{if(this.cz){var hl=0;for(i=0;i<this.items.length;i++){var el=this.items[i];hl=Math.min((Math.max(hl,el.scrollWidth)),this.tree.cc);}for(i=0;i<this.items.length;i++){this.items[i].style.pixelWidth=hl;}this.style.pixelWidth=this.cs.style.pixelWidth=hl+(Hu*2);}for(i=0;i<this.items.length;i++){var el=this.items[i];if(el.index){var dt=(el.siblingBelow.scrollHeight+Hw);
el.style.pixelTop=el.siblingBelow.style.pixelTop+dt;}else el.style.pixelTop=0;if(el.hg){if(this.tree.R){el.hg.style.pixelLeft=(Ht+HD);}else{el.hg.style.pixelLeft=el.style.pixelWidth-(Ht+HD+HC);}if(typeof(Hgi)!="number"){if(Hgi=="bottom"){el.hg.style.pixelTop=Math.max((el.scrollHeight-Hl5-Ht),Ht);}else{el.hg.style.pixelTop=parseInt(Math.max((el.scrollHeight-Hl5),Ht)/2);}}}}this.style.pixelHeight=this.cs.style.pixelHeight=(el.style.pixelTop+el.scrollHeight+(Hu*2));}this.hm=this.style.pixelHeight;this.hn=true;
...
}}this.child=Hg.document.all(this.jd);if(this.child){this.child.ct=this.menu;this.child.cu=this;}}if(this.dh&&!this.G){this.onclick=HWm;if(HT)this.style.cursor="hand";}if(window.HM_o_RolloverImages){var l1=this.all.tags("IMG");for(var i=0;i<l1.length;i++){var l4=l1[i].id;if(typeof(l4)!="undefined")l4=l4.replace(Hl2,"$1");if(l4&&HM_o_RolloverImages[l4]){if(!this.lv)this.lv=[];l1[i].ed=l1[i].src;l1[i].ee=HM_o_RolloverImages[l4];this.lv[this.lv.length]=l1[i];}}}this.HXp();}
function HM_f_PopUp(eg){if(!Hat)return;if(Hax)return;if(!Hay&&!Hbi){if(!HW5())return;}eg=eg.replace("elMenu",Hab);var eh=Hg.document.all(eg);var ma=false;if(!eh&&!Hlh){if(Hay||!Hbi)return;var lm=parseInt(eg.replace(Hab,""));if(isNaN(lm))return;eh=Hg.document.all(Hab+lm);if(eh)return;eh=HXH(lm);if(eh)ma=true;}if(!eh)return;if(eh.tree.G){var ej="return HWe('"+eg+"',false);";var ei=event.srcElement;ei.onclick=new Function(ej);}else return HWe(eg,ma);}
function HWe(eg,l9){if(!Hat||!Hbi)return true;if(Hax)return true;var eh=Hg.document.all(eg);if(!eh)return true;Hbg=eh;if(Hbg.tree.G&&event.type!="click")return true;var mouse_x_position,mouse_y_position;HWn();Hbg.cv=false;Hbg.tree.db=Hbg;if(!H4){Hbg.mouseX=mouse_x_position=(event.clientX+document.body.scrollLeft);Hbg.mouseY=mouse_y_position=(event.clientY+document.body.scrollTop);}else{switch(H5){case "top":Hbg.mouseX=mouse_x_position=(event.clientX+Hg.document.body.scrollLeft);
...
Hbg.mouseY=mouse_y_position=(event.clientY+Hg.document.body.scrollTop);break;}}Hbg.ir=Hbg.xPos=(Hbg.tree.ca!=null)?eval(Hbg.tree.ca):mouse_x_position;Hbg.is=Hbg.yPos=(Hbg.tree.cb!=null)?eval(Hbg.tree.cb):mouse_y_position;Hbg.isOn=true;if(l9){var en="if(Hg&&Hg."+Hbg.id+"&&";en+="window.HXR)";en+="HXR('"+eg+"')";setTimeout(en,10);}else HXR(eg);return false;}
function HXR(eg){if(!Hat||!Hbi)return true;if(Hax)return true;var eh=Hg.document.all(eg);if(!eh)return true;Hbg=eh;if(!Hbg.hn)Hbg.HXq(false);if(!Hbg.isOn)return true;if(Hbg.bq){Hbg.style.pixelHeight=Hbg.hm;Hbg.HXl();}Hbg.HXc();Hbg.moveTo(Hbg.xPos,Hbg.yPos);Hbg.HXb(true);}
function HWf(){if(Hax)return;if(this.contains(Hg.event.fromElement))return;if(!this.tree.db){this.tree.db=this;}if(this.tree.db==this)HWn(this);this.isOn=true;Hbj=true;Hbg=this;if(HV&&(Hku!=Hbg))HWx();if(this.bc)clearTimeout(this.bc);if(HF){if(this!=this.tree.db){var er=this.ct;if(er.bn&&er.bn.dj)er.bn.HXP(false);var kk=this.cu;if(kk.dj)kk.HXP(true);er.bn=kk;}}}
function HWg(){if(Hax)return;if(this.contains(Hg.event.toElement))return;this.isOn=false;Hbj=false;if(HO)window.status="";var Items=this.cs.children;var ek=Items.length;var el;for(var i=0;i<ek;i++){el=Items[i];clearTimeout(el.k4);el.k4=null;}if(!Hk3){clearTimeout(Hbd);Hbd=null;var en="var hp=HW9('"+this.id+"');if(hp&&hp.HXd)hp.HXd()";Hbd=setTimeout(en,Han);}}
function HWZ(){if(!this.menu.isOn||this.menu.bn!=this)return;if(!this.child.hn)this.child.HXq(false);if(this.menu.style.visibility=="hidden")return;if(this.tree.ch&&(this.menu==this.tree.cm)){this.child.xPos=this.menu.style.pixelLeft+this.style.pixelLeft;this.child.yPos=this.menu.style.pixelTop+this.menu.offsetHeight-(Hu);}else{if(this.I==null){this.dw=this.style.pixelWidth;if(!this.menu.cw||(this.menu.cw&&this.hi))this.dw+=Hu;this.dx=(!this.menu.cw||(this.menu.cw&&this.index==0))?Hu:Hw;
this.I=(parseInt((HK!=null)?(HK*this.dw):HI));}this.oL=this.menu.style.pixelLeft+this.offsetLeft+Hu;this.oT=this.menu.style.pixelTop+this.offsetTop;if(HV){this.oT+=this.menu.cs.top;if(H2&&this.menu.hd)this.oT+=Hk2;}if(this.tree.R){this.child.xPos=((this.oL-this.dx)+this.I)-this.child.style.pixelWidth;}else{this.child.xPos=(this.oL+this.dw)-this.I;}this.child.yPos=this.oT+HJ+Hu;}if(!this.tree.ch||this.menu!=this.tree.cm){if(this.child.bq){this.child.style.pixelHeight=this.child.hm;}this.child.HXc();
}this.child.moveTo(this.child.xPos,this.child.yPos);this.menu.bm=true;this.menu.eo=this.child;this.child.HXb(true);}
...
function HWh(){if(Hax)return;if(this.contains(Hg.event.fromElement))return;var ep=this.menu;if(HF){if(ep.bn&&ep.bn!=this&&ep.bn.dj){ep.bn.HXP(false);}}if(Hg.event.srcElement.id=="HM_ImMore")return;if(this.dj){this.HXP(true);}if(HO)window.status=this.dh;this.menu.bn=this;var Items=ep.cs.children;var ek=Items.length;var el;for(var i=0;i<ek;i++){el=Items[i];clearTimeout(el.k4);el.k4=null;}var en="if(Hg&&Hg."+this.id+"&&";en+="Hg."+this.id+".HXa)";en+="Hg."+this.id+".HXa(true)";this.k4=setTimeout(en,this.dn);}
function HWi(onover){if(Hax)return;if(this.menu.bm){if(this.menu.eo==this.child&&this.menu.eo.bm)this.menu.eo.HXh(this);else this.menu.HXh(this);}if((this.G&&!onover)||(this.dc&&!this.G)){if(!this.child){Hbf=this.tree;Hbg=this.menu;Hbh=this;this.child=HWc(this.menu.count+"_"+(this.index+1));this.child.cu=this;Hbf=this.tree;Hbg=this.menu;Hbh=this;var en="if(Hg&&Hg."+this.id+"&&";en+="Hg."+this.id+".HX9)";en+="Hg."+this.id+".HX9()";this.k4=setTimeout(en,10);}else this.HX9();}}
function HWj(){if(Hax)return;if(this.contains(Hg.event.toElement))return;if(this.dj){if(!(HF&&this.child&&this.child.contains(Hg.event.toElement)))this.HXP(false);}}
...
function HWl(){var et=H7;var eu=Hg.document.body.scrollLeft;var ev=Hg.document.body.scrollTop;var ew=Hg.document.body.clientWidth;var ex=Hg.document.body.clientHeight;var ey=(eu+ew)-et;var ez=(ev+ex)-et;var Fa=this.xPos;var Fb=Fa+this.style.pixelWidth;if((this.yPos<ev)&&(this.cv||this.tree.lf))this.yPos=ev;var Fc=this.yPos+this.style.pixelHeight;if(this.cv){var Fd=this.cu.oL;}if((Fb>ey)&&(this.cv||this.tree.le)){if(this.cv){this.xPos=((Fd-this.cu.dx)+this.cu.I)-this.style.pixelWidth;}else{var dif=Fb-ey;
this.xPos-=dif;}this.xPos=Math.max(0,this.xPos);}if((Fc>ez)&&(this.cv||this.tree.lf)){var dif=Fc-ez;this.yPos-=dif;Fc=ez;}if((Fa<eu)&&(this.cv||this.tree.le)){if(this.cv){this.xPos=(Fd+this.cu.dw)-this.cu.I;Fb=this.xPos+this.style.pixelWidth;if(Fb>ey)this.xPos-=(Fb-ey);}else{this.xPos=0}}if(HV){if(((this.yPos<ev)&&(this.cv||this.tree.lf))||(Fc>ez)){if(this.yPos<ev){if(!H4||H5!="top"){this.yPos=(ev+et);var Fj=(et*2);}else{this.yPos=ev;var Fj=et;}this.style.pixelHeight=(Math.min((this.hm),(Math.max(ex,
((Hk2*2)+H3))))-Fj);}else{this.style.pixelHeight=Math.min((this.hm),Math.max((ez-this.yPos),((Hk2*2)+H3)));}if(!this.bq)this.HXj();this.HXl();}else{if(this.bq)this.HXl();}}}
...
function HWv(){var hs=(this.style.pixelWidth-(Hu*2));var ht=Hk2-Hu;var hu=(Hu+"px "+this.tree.v+" "+Hhf);var hv="STYLE='position:absolute;visibility:hidden;top:0;left:0;text-align:center;padding-top:"+((Hk2-Hu-H1)/2)+"px;";var hw=";width:"+hs+";height:"+ht+";background-color:"+Hk1+";'><IMG VSPACE=0 HSPACE=0 SRC='";var hx="'></DIV>";var hy="border-bottom:"+hu;var hz="border-top:"+hu;var je="<DIV ID='Hik' "+hv+hy+hw+Hkz+hx;var ia="<DIV ID='HM_ScrollBot' "+hv+hz+hw+Hky+hx;this.insertAdjacentHTML("BeforeEnd",
je+ia);this.Fs=this.children.item("Hik");this.Fs.menu=this;this.Ft=this.children.item("HM_ScrollBot");this.Ft.menu=this;if(Hkw){this.Fs.onmouseover=HXA;this.Fs.onmouseout=HWx;this.Ft.onmouseover=HXB;this.Ft.onmouseout=HWx;}else{this.Fs.onmousedown=HXA;this.Ft.onmousedown=HXB;}this.bq=true;}
